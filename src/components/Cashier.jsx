...
export default function Cashier() {
  ...
  const summary = useMemo(() => {
    ...
    const tax = cart.reduce((t, i) => t + i.price * i.qty * i.taxRate, 0) * ((sub - disc) / (sub || 1));
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-4">
        ...
        <div className="mt-3 text-xs text-gray-500">Tap to add to cart. Taxes auto-applied.</div>
      </div>
    </div>
  );
}
